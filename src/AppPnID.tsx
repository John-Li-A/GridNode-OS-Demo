...
import { Fluid3DViewport } from '@/components/Fluid3DViewport';
import type { Equipment, ValveState, SensorData, SystemStatus } from '@/types/pnid';
...
const INITIAL_EQUIPMENT: Equipment[] = [
  {
    id: 'HEX-101', name: 'HEX-101', code: 'PLATE HEAT EXCHANGER',
    description: '板式换热器，用于冷却液与热液的热量交换',
    x: 440, y: 220, width: 80, height: 100, status: 'normal',
    params: {
  ...
  {
    id: 'P-101', name: 'P-101', code: 'CENTRIFUGAL PUMP',
    description: '离心泵，提供冷却液循环动力',
    x: 440, y: 380, width: 50, height: 50, status: 'normal',
    params: {
  ...
  {
    id: 'EXP-101', name: 'EXP-101', code: 'EXPANSION TANK',
    description: '膨胀罐，用于容纳冷却液热膨胀容积',
    x: 100, y: 400, width: 40, height: 60, status: 'normal',
    params: {
      'Level': { value: 65, unit: '%' },
  ...
  {
    id: 'VFD-101', name: 'VFD', code: 'VFD CONTROLLER',
    description: '变频驱动器，控制离心泵转速',
    x: 440, y: 460, width: 40, height: 30, status: 'normal',
    params: {
  ...
  {
    id: 'Y-101', name: 'Y-101', code: 'STRAINER',
    description: 'Y型过滤器，过滤冷却液中的杂质',
    x: 320, y: 340, width: 25, height: 25, status: 'normal',
    params: {
  ...
  {
    id: 'NRV-101', name: 'NRV-101', code: 'CHECK VALVE',
    description: '止回阀，防止冷却液倒流',
    x: 600, y: 340, width: 20, height: 20, status: 'normal',
    params: {
  ...
  {
    id: 'BDV-101', name: 'BDV-101', code: 'BLOWDOWN VALVE',
    description: '排污阀，用于系统排污维护',
    x: 100, y: 500, width: 20, height: 20, status: 'normal',
    params: {
...
function PnIDPage() {
  ...
  const [systemStatus, setSystemStatus] = useState<SystemStatus>({
    coolingLoad: 100, totalFlow: 85, deltaT: 44.9, pumpSpeed: 62.5,
    systemPressure: 350, alarmStatus: 'NORMAL',
  });
  const [hoveredPipe, setHoveredPipe] = useState<string | null>(null);
  const [deviceDetail, setDeviceDetail] = useState<{ equipment: Equipment; visible: boolean }>({
    equipment: { id: '', name: '', code: '', description: '', x: 0, y: 0, width: 0, height: 0, status: 'normal', params: {} },
    visible: false,
  });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setSystemStatus(prev => {
        ...
        return { ...prev, coolingLoad: newLoad, totalFlow: newFlow, pumpSpeed: Math.round((62.5 + (Math.random() - 0.5)) * 10) / 10, alarmStatus: alarm };
      });
    }, 1000);
  ...
  const updateValve = useCallback((valveId: string, newOpenPercent: number) => {
    setValves(prev => prev.map(v => v.id === valveId ? { ...v, openPercent: newOpenPercent } : v));
    const flowFactor = newOpenPercent / 100;
    setSystemStatus(prev => ({ ...prev, totalFlow: Math.round(85 * flowFactor), systemPressure: Math.round(350 * (0.5 + flowFactor * 0.5)) }));
  }, []);

  const showDeviceDetail = useCallback((eq: Equipment) => {
    setDeviceDetail({ equipment: eq, visible: true });
    ...
    <div style={{ width: '100vw', height: '100vh', backgroundColor: '#0a0a0a', display: 'flex', flexDirection: 'column', overflow: 'hidden', paddingTop: '26px', boxSizing: 'border-box' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        <div style={{ width: '60%', height: '100%', overflow: 'auto', backgroundColor: '#0a0a0a', borderRight: '1px solid #1a1a2e' }}>
          <PnIDDiagram
            sensors={sensors} valves={valves} equipment={equipment}
            hoveredPipe={hoveredPipe} onPipeHover={setHoveredPipe}
            onEquipmentClick={showDeviceDetail} onValveClick={setActiveValve}
            selectedEquipment={selectedEquipment}
          />
        </div>
        <div style={{ width: '40%', height: '100%' }}><Fluid3DViewport /></div>
      </div>
      <DeviceDetailOverlay equipment={deviceDetail.equipment} visible={deviceDetail.visible} onClose={hideDeviceDetail} />
      <ValveControlPanel valve={activeValve} onClose={() => setActiveValve(null)} onChange={updateValve} />
      <div style={{ height: '28px', backgroundColor: '#0a0a0a', borderTop: '1px solid #1a1a2e', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', fontFamily: '"Consolas",monospace', fontSize: '10px', flexShrink: 0 }}>
        <div style={{ display: 'flex', gap: '24px' }}>
          <span><span style={{ color: '#5a5a6a' }}>COOLING LOAD: </span><span style={{ color: '#00aaff', fontWeight: 600 }}>{systemStatus.coolingLoad} %</span></span>
          <span><span style={{ color: '#5a5a6a' }}>TOTAL FLOW: </span><span style={{ color: '#00aaff', fontWeight: 600 }}>{systemStatus.totalFlow} L/min</span></span>
          <span><span style={{ color: '#5a5a6a' }}>DELTA T: </span><span style={{ color: '#00aaff', fontWeight: 600 }}>{systemStatus.deltaT} °C</span></span>
          <span><span style={{ color: '#5a5a6a' }}>PUMP SPEED: </span><span style={{ color: '#00aaff', fontWeight: 600 }}>{systemStatus.pumpSpeed} Hz</span></span>
          <span><span style={{ color: '#5a5a6a' }}>SYS PRESSURE: </span><span style={{ color: '#00aaff', fontWeight: 600 }}>{systemStatus.systemPressure} kPa(g)</span></span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <span style={{ color: '#5a5a6a' }}>ALARM STATUS:</span>
          <span style={{ color: systemStatus.alarmStatus === 'NORMAL' ? '#00ff66' : systemStatus.alarmStatus === 'WARNING' ? '#ffcc00' : '#ff3333', fontWeight: 700 }}>{systemStatus.alarmStatus}</span>
          <span style={{ width: '7px', height: '7px', borderRadius: '50%', backgroundColor: systemStatus.alarmStatus === 'NORMAL' ? '#00ff66' : systemStatus.alarmStatus === 'WARNING' ? '#ffcc00' : '#ff3333', boxShadow: `0 0 5px ${systemStatus.alarmStatus === 'NORMAL' ? '#00ff66' : systemStatus.alarmStatus === 'WARNING' ? '#ffcc00' : '#ff3333'}` }} />
        </div>
      </div>
    </div>
  );
